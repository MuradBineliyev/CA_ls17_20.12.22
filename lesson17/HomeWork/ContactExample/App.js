...
import SectionListContact from './companents/SectionListContact';
...
export default function App() {
  const datas = [
    ...
    alphabet.forEach(e => {
      let data = datas.filter(x => x.firstName.toLocaleUpperCase().startsWith(e));
      const item = {
        title: e,
        data : data
      }
  ...
  return (
    <View style={styles.container}>
      <SectionList
      sections={contacts}
      renderItem={({ item }) => SectionListContact({item})}
      renderSectionHeader={({ section: { title } }) => (
        <Text style = {styles.textTitle}>{title}</Text>
        )}
        stickySectionHeadersEnabled = {true}
    />
      {/* <StatusBar style="true" hidden = {false} /> */}
    </View>
  );
}
